...
#the periodic real-valued function f(x,y) with period equal to P
f = lambda x, y: np.cos((2./3.) * x) - np.sin((2./3.) * y) / 2.
...
def fit_func_by_fourier_series_with_complex_coeffs(x, y, C):
    result = 0. + 0.j
    L = int((len(C) - 1) / 2)
...
for x in x_range:
    for y in y_range:
        xyz_approx.append([x, y, fit_func_by_fourier_series_with_complex_coeffs(x, y, C)])
